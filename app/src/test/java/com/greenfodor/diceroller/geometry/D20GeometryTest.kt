...
class D20GeometryTest {
    ...
    fun `getFaceRotation orients first vertex to top of screen`() {
        (0 until 20).forEach { faceIndex ->
            ...
            val indices = IcosahedronGeometry.faceIndices[faceIndex]
            val v0 = IcosahedronGeometry.vertices[indices[0]]
            val center = (v0 + IcosahedronGeometry.vertices[indices[1]] + IcosahedronGeometry.vertices[indices[2]]) * (1f / 3f)
            ...
            assertEquals("Face $faceIndex: Top vertex should be horizontally centered", centerRotated.x, v0Rotated.x, 0.01f)
            assertTrue("Face $faceIndex: Top vertex should be above center (smaller Y)", v0Rotated.y < centerRotated.y)
        }
    }
}
